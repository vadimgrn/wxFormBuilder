...
void CppCodeGenerator::GenDarkModeHelper()
{
    ...
    m_source->WriteLn(wxT("auto pick_resname = [dark = wxSystemSettings::GetAppearance().IsDark()] (const auto& name, const auto& name_dark) noexcept -> decltype(auto)"));
    m_source->WriteLn(wxT("{"));
    ...
    m_source->WriteLn(wxT("auto pick_resname = [] (const auto& name, const auto&) noexcept -> decltype(auto) { return name; };"));
